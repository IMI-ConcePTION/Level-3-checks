...
flowchart_vx<-fread(paste0(vacc_dir,"Masked/", "vaccines_flowchart_masked.csv"))
datatable(flowchart_vx, options = list(scrollX=T))
} else {
flowchart_vx<-fread(paste0(vacc_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_vaccines_flowchart_masked.csv"))
datatable(flowchart_vx, options = list(scrollX=T))
}
```
...
if (subpopulations_present=="No"){
description_vx<-fread(paste0(vacc_dir,"Masked/", "vaccines_description_masked.csv"))
datatable(description_vx, options = list(scrollX=T))
} else {
  description_vx<-fread(paste0(vacc_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_vaccines_description_masked.csv"))
datatable(description_vx, options = list(scrollX=T))
}
```
...
if (subpopulations_present=="No"){
  if(sum(grepl(list.files(paste0(vacc_dir,"Masked/")), pattern="vaccines_completeness_masked.csv"))>0){
tab15_vacc<-fread(paste0(vacc_dir,"Masked/", "vaccines_completeness_masked.csv"))
if (tab15_vacc[,.N]>0){
datatable(tab15_vacc, options = list(scrollX=T))
  ...
  tab15_vacc<-NULL
    ...
    if(sum(grepl(list.files(paste0(vacc_dir,subpopulations_names[a], "/Masked/")), pattern="vaccines_completeness_masked.csv"))>0){
  tab15_vacc<-fread(paste0(vacc_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_vaccines_completeness_masked.csv"))
  if (tab15_vacc[,.N]>0){
datatable(tab15_vacc, options = list(scrollX=T))
  } else {
    tab15_vacc<-NULL
